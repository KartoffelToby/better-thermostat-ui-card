// Minimal helpers copied/derived from the HA core frontend implementation to
// avoid bundling the whole HA frontend codebase while keeping the real logic.
import { UNAVAILABLE, UNKNOWN } from "./climate";
...
function computeGroupDomain(stateObj: any) {
  ...
  const uniqueDomains = [
    ...new Set(entityIds.map((id: string) => computeDomain(id))),
  ];
...
export const stateColorProperties = (
  ...
  if (domain === "group") {
    ...
    if (
      groupDomain &&
      STATE_COLORED_DOMAIN.has(groupDomain as unknown as string)
    ) {
